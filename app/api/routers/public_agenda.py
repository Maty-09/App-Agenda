...
def agenda_publica(tenant_id: str, response: Response, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "embed_script": f"{base_url}/api/v1/public/{tenant.id}/agenda/widget.js",
    }


@router.get("/{tenant_id}/agenda/disponibilidad")
def disponibilidad_publica(tenant_id: str, response: Response, fecha: str = Query(..., pattern=r"^\d{4}-\d{2}-\d{2}$"), db: Session = Depends(get_db)):
...
def widget_agenda(tenant_id: str, db: Session = Depends(get_db)):
  ...
  script.parentNode.insertBefore(frame,script.nextSibling);
    ...
    return RawResponse(script, media_type="application/javascript", headers={"Access-Control-Allow-Origin": "*"})
